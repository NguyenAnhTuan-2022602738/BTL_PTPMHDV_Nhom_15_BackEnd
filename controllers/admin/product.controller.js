...
module.exports.deleteItem = async (req, res) => {
    ...
    try {
        ...
        await Car_items.updateOne({ _id: id},{deleted: true, deletedAt: new Date()});
...
module.exports.changeMulti = async (req, res) => {
    ...
    switch(type){
        case "delete-multi":
            await Car_items.updateMany({_id: {$in: ids}}, {deleted: true, deletedAt: new Date()});
...
module.exports.createPost = async (req, res) => {
    ...
    try {
        ...
        if (existingCar) {
            // Nếu trùng, gửi thông báo lỗi
            req.flash("error", "Trùng sản phẩm!");
            res.redirect("back");
            return;
        }

        // Kiểm tra nếu có ảnh cũ (từ client), gộp chúng vào mảng ảnh
        formData.imageUrl = req.body.existingImages || []; // existingImages là mảng ảnh từ client

        // Thêm các ảnh mới được tải lên vào mảng `imageUrl`
        const newImages = req.files ? req.files.map(file => `/uploads/${file.filename}`) : [];
        formData.imageUrl = [...formData.imageUrl, ...newImages];
...
module.exports.importCarItems = (req, res) => {
    ...
    fs.readFile(filePath, "utf8", async (err, data) => {
        ...
        try {
            ...
            const existingItemsSet = new Set(existingItems.map(item => `${item.version}-${item.name}-${item.brand}`));
            ...
            for (const item of carItems) {
                ...
                const newItem = {
                    ...
                    canhBaoPhuongTienCatNgang: item.canhBaoPhuongTienCatNgang || 'Đang cập nhật',
                    canhBaoApSuatLop: item.canhBaoApSuatLop || 'Đang cập nhật',
                    heThongChongBoCungPhanh: item.heThongChongBoCungPhanh || 'Đang cập nhật',
                ...
                if (!existingItemsSet.has(itemKey)) {
                    ...
                    req.flash('error', `Xe ${newItem.name}-${newItem.version} đã tồn tại trong hệ thống!`);
            ...
            if (itemsToInsert.length > 0) {
                ...
                res.redirect(`${systemConfig.prefixAdmin}/car_items/detail/${fisrtItemId}`);
                // res.redirect("back");
            }
