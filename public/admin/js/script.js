...
if (uploadImage) {
    ...
    const existingImages = JSON.parse(uploadImagesPreview.getAttribute('data-existing-images') || '[]');
    ...
    const collectImageUrls = () => {
        const existingImageElements = document.querySelectorAll('.box-image-preview img');
        const imageUrls = [];
    ...
    const saveFiles = () => {
        ...
        // Ví dụ: gửi finalFilesArray đến server qua fetch hoặc AJAX
    };
